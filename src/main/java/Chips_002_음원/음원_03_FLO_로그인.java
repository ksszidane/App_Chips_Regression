...
String AccessToken;
	...
	public void TC_0228_Chips_FLO로그인_셔플해줘_확인(Method method) throws Exception {
		...
		test.log(Status.INFO, "W, 셔플 해줘 - 발화");
		util.SWFsendPost("셔플 해줘", ServerName, AccessToken);
		...
		util.context("WEBVIEW_com.skt.aidev.nugufriends");
		util.waitForPageLoaded();
		boolean 좋아요버튼활성화 = util.isElementPresent(By.xpath(xPath.랜덤재생On_web));
		Assert.assertTrue(좋아요버튼활성화);
		util.context("NATIVE_APP");
	...
	public void TC_0229_Chips_FLO로그인_셔플해제_확인(Method method) throws Exception {
		...
		test.log(Status.INFO, "W, 랜덤 그만해 - 발화");
		util.SWFsendPost("랜덤 그만해", ServerName, AccessToken);
		...
		util.context("WEBVIEW_com.skt.aidev.nugufriends");
		util.waitForPageLoaded();
		boolean 좋아요버튼활성화 = util.isElementPresent(By.xpath(xPath.랜덤재생Off_web));
		Assert.assertTrue(좋아요버튼활성화);
		util.context("NATIVE_APP");
