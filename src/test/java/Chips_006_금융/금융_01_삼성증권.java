...
public class 금융_01_삼성증권 extends Chips_TestCase {
	...
	public void TC_0959_1_Chips_삼성증권_종합주가지수_장전_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "코스닥은"));
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "코스피는"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "포인트로 마감되었습니다."));
		} else if (장상태 == "장중") {
	...
	public void TC_0959_2_Chips_삼성증권_종합주가지수_장중_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "포인트를 기록 중 입니다."));
		} else if (장상태 == "장종") {
	...
	public void TC_0965_Chips_삼성증권_종합주가지수_코스닥_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			test.log(Status.INFO, "장전 주가지수 TTS 확인");
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "어제 코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "코스피은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "포인트로 마감되었습니다."));
		} else if (장상태 == "장전" && util.getDayOfWeek(0) == "월") {
			test.log(Status.INFO, "장전 주가지수 TTS 확인");
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "어제는 증시 휴장일이었어요"));
			...
			test.log(Status.INFO, "장중 주가지수 TTS 확인");
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "현재 코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "코스피는"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "포인트를 기록 중 입니다."));
		} else if (장상태 == "장종") {
			test.log(Status.INFO, "장종 주가지수 TTS 확인");
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "오늘 코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "코스피는"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "포인트로 마감되었습니다."));
		}
	}
	...
	public void TC_0976_Chips_삼성증권_국외지수_확인(Method method) throws Exception {
		...
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "일본 니케이지수는"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "중국 상해종합지수는"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "퍼센트"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "포인트"));
	...
	public void TC_0984_Chips_삼성증권_종목별주가_장중_정지_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "직전 종가는"));
		} else if (장상태 == "장종") {
			test.log(Status.INFO, "현재 [장종]상태로 장중상태 Test Skip");
			throw new SkipException("현재 [장종]상태로 장중상태 Test Skip");
	...
	public void TC_0992_Chips_삼성증권_종목추천_확인(Method method) throws Exception {
		...
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, "주식은 원금손실의 위험이 있으니, 투자 전 자세한 설명을 듣고 결정을 내리시기 바래요."));
