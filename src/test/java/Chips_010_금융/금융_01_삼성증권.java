...
public class 금융_01_삼성증권 extends Chips_TestCase {
	...
	public void TC_0953_Chips_삼성증권_도움말_확인(Method method) throws Exception {
		...
		Assert.assertTrue(util.TTS_Assertfunc_EqualsSet(nuguqa001, Chips_001, ServerName, Place, Service, data.삼성증권도움말_set));
	}
	...
	public void TC_0959_1_Chips_삼성증권_종합주가지수_장전_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "어제 코스피는"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트로 마감되었습니다."));
		} else if (장상태 == "장전" && util.getDayOfWeek(0) == "월") {
			test.log(Status.INFO, "W, 현재 주가지수 알려줘 - 발화");
			util.SWFsendPost("현재 주가지수 알려줘", Chips_001, ServerName, AccessToken);
			
			test.log(Status.INFO, "장전 주가지수 TTS 확인");
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "어제는 증시 휴장일이었어요"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "코스피는"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트로 마감되었습니다."));
		} else if (장상태 == "장중") {
	...
	public void TC_0959_2_Chips_삼성증권_종합주가지수_장중_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "현재 코스피는"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트를 기록 중 입니다."));
		} else if (장상태 == "장종") {
	...
	public void TC_0959_3_Chips_삼성증권_종합주가지수_장종_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "오늘 코스피는"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트로 마감되었습니다."));
		}
	}
	...
	public void TC_0960_1_Chips_삼성증권_종합주가지수_장전_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			test.log(Status.INFO, "장전 주가지수 TTS 확인");
			String tts = util.TTS_JsonParsing(nuguqa001, Chips_001, ServerName, Place, Service);
	...
	public void TC_0965_Chips_삼성증권_종합주가지수_코스닥_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			test.log(Status.INFO, "장전 주가지수 TTS 확인");
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "어제 코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "코스피은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트로 마감되었습니다."));
		} else if (장상태 == "장전" && util.getDayOfWeek(0) == "월") {
			test.log(Status.INFO, "장전 주가지수 TTS 확인");
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "어제는 증시 휴장일이었어요"));
			...
			test.log(Status.INFO, "장중 주가지수 TTS 확인");
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "현재 코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "코스피는"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트를 기록 중 입니다."));
		} else if (장상태 == "장종") {
			test.log(Status.INFO, "장종 주가지수 TTS 확인");
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "오늘 코스닥은"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "코스피는"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트로 마감되었습니다."));
		}
	}
	...
	public void TC_0969_Chips_삼성증권_미지원기간_과거_확인(Method method) throws Exception {
		...
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "가장 최근의 종합주가지수 정보만 알려드릴 수 있어요"));
	...
	public void TC_0974_Chips_삼성증권_미지원기능_확인(Method method) throws Exception {
		...
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "외국인, 기관등의 매입, 매도를 구분해서 확인하면 종목을 알려 드릴수 있어요."));
	...
	public void TC_0976_Chips_삼성증권_국외지수_확인(Method method) throws Exception {
		...
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "미국 다우지수는"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "나스닥은"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "일본 니케이지수는"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "중국 상해종합지수는"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "퍼센트"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트"));
	...
	public void TC_0978_Chips_삼성증권_특정국가지수_홍콩지수_확인(Method method) throws Exception {
		...
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "홍콩 항셍지수는"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "퍼센트"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트"));
	...
	public void TC_0979_Chips_삼성증권_특정국가지수_일본지수_확인(Method method) throws Exception {
		...
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "일본 니케이 지수는"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "퍼센트"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트"));
	...
	public void TC_0980_Chips_삼성증권_특정국가지수_대만지수_확인(Method method) throws Exception {
		...
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "대만 가권 지수는"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "퍼센트"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "포인트"));
	...
	public void TC_0981_Chips_삼성증권_종목별주가_장중_상승_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "현재 "+상승종목));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "퍼센트 상승한"));
		} else if (장상태 == "장종") {
			test.log(Status.INFO, "현재 [장종]상태로 장중상태 Test Skip");
			throw new SkipException("현재 [장종]상태로 장중상태 Test Skip");
	...
	public void TC_0982_Chips_삼성증권_종목별주가_장중_하락_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "현재 "+하락종목));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "퍼센트 하락한"));
		} else if (장상태 == "장종") {
			test.log(Status.INFO, "현재 [장종]상태로 장중상태 Test Skip");
			throw new SkipException("현재 [장종]상태로 장중상태 Test Skip");
	...
	public void TC_0983_Chips_삼성증권_종목별주가_장중_보합_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "현재 "+보합종목));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "주가는 보합인"));
		} else if (장상태 == "장종") {
			test.log(Status.INFO, "현재 [장종]상태로 장중상태 Test Skip");
			throw new SkipException("현재 [장종]상태로 장중상태 Test Skip");
	...
	public void TC_0984_Chips_삼성증권_종목별주가_장중_정지_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, 정지종목));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "거래정지 종목으로"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "직전 종가는"));
		} else if (장상태 == "장종") {
			test.log(Status.INFO, "현재 [장종]상태로 장중상태 Test Skip");
			throw new SkipException("현재 [장종]상태로 장중상태 Test Skip");
	...
	public void TC_0985_Chips_삼성증권_종목별주가_장종_상승_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "오늘 "+상승종목));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "퍼센트 상승한"));
	...
	public void TC_0986_Chips_삼성증권_종목별주가_장종_하락_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "오늘 "+하락종목));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "퍼센트 하락한"));
	...
	public void TC_0987_Chips_삼성증권_종목별주가_장종_보합_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "오늘 "+보합종목));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "보합"));
	...
	public void TC_0989_Chips_삼성증권_종목별주가_장전_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "SK텔레콤의 어제 종가는"));
	...
	public void TC_0990_Chips_삼성증권_종목별주가_장전_전일휴일_확인(Method method) throws Exception {
		...
		if (장상태 == "장전") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "금요일 기준"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "SK텔레콤의 종가는"));
	...
	public void TC_0991_Chips_삼성증권_종목별주가_휴장일_확인(Method method) throws Exception {
		...
		if (util.getDayOfWeek(0) == "토") {
			...
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "오늘은 증시 휴장일입니다"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "금요일 기준"));
			Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "SK텔레콤의 종가는"));
	...
	public void TC_0992_Chips_삼성증권_종목추천_확인(Method method) throws Exception {
		...
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "삼성증권에서"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "Premium List ten으로 선정한 종목중에"));
		Assert.assertTrue(util.TTS_Assertfunc(nuguqa001, Chips_001, ServerName, Place, Service, "주식은 원금손실의 위험이 있으니, 투자 전 자세한 설명을 듣고 결정을 내리시기 바래요."));
